...
@section('title', 'Join an Instructor - Setting')
@section('maincontent')
<?php
        ...
        <div class="col-lg-12">
            <div class="card dashboard-card m-b-30">
                <div class="card-header">
                    <h5 class="card-title">{{ __('Join an Instructor') }}</h5>
                </div>
                <div class="card-body">
                    <form class="form" action="{{ route('join.update') }}" method="POST" novalidate
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                ...
                                <img src="{{ url('/images/joininstructor/'.$setting->img) }}" height="100px;" width="100px;" title="{{ __('Image')}}" />
                                @else
                                <img src="{{ Avatar::create($setting->text)->toBase64() }}" alt="{{ __('Image')}}" class="img-fluid">
                                @endif
                            </div>
                            <div class="form-group col-md-4">
                            ...
                            <div class="form-group col-md-6">
                                <label class="text-dark">{{ __('Details') }}:</label>
                                <input name="detail" value="{{ $setting->detail }}" autofocus="" type="detail"
                                    class="{{ $errors->has('text') ? ' is-invalid' : '' }} form-control"
                                    placeholder="{{ __('Enter Details')}}" required="">
                                <div class="invalid-feedback">
                                    {{ __('Please enter Details!') }}.
                                </div>
                            </div>
                        </div>
