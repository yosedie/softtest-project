                    ...
                    <div class="view-block">
                        <div class="view-img">
                            @if($c['preview_image'] !== NULL && $c['preview_image'] !== '')
                                <a href="{{ route('user.course.show',['slug' => $c->slug ]) }}"><img src="{{ asset('images/course/'.$c['preview_image']) }}" alt="{{ __('Course') }}" class="img-fluid" >
                                </a>
                            @else
                                <a href="{{ route('user.course.show',['slug' => $c->slug ]) }}"><img src="{{ Avatar::create($c->title)->toBase64() }}" alt="{{ __('course') }}"class="img-fluid">
                                </a>
                            @endif
                        </div>
                        ...
                        <div class="view-user-img">
                            ...
                            <a href="{{ route('all/profile',$c->user->id) }}" title="{{ __('course') }}"><img src="{{ asset('images/user_img/'.$c->user['user_img']) }}" class="img-fluid user-img-one" alt="{{ __('course') }}"></a>
                            @else
                            <a href="{{ route('all/profile',$c->user->id) }}" title="{{ __('course') }}"><img src="{{ asset('images/default/user.png') }}" class="img-fluid user-img-one" alt="{{ __('course') }}"></a>
                            @endif
                         
                        </div>
                        <div class="view-dtl">
                            <div class="view-heading"><a href="{{ route('user.course.show',['slug' => $c->slug ]) }}">{{ str_limit($c['title'], $limit = 35, $end = '...') }}</a></div>
                            <div class="user-name">
                ...
                <div id="prime-next-item-description-block{{$c->id}}" class="prime-description-block">
                    <div class="prime-description-under-block">
                        <div class="prime-description-under-block">
                            ...
                            <div class="main-des">
                                <p>{{__('Last Updated:')}} {{ date('jS F Y', strtotime($c->updated_at)) }}</p>
                            </div>
                                        ...
                                        @if($c->type == 1)
                                            @if(Auth::check())
                                                @if(Auth::User()->role == "admin")
                                                    <div class="protip-btn">
                                                        <a href="{{ route('course.content',['slug' => $c->slug ]) }}" class="btn btn-secondary" title="course">{{__('Go To Course')}}</a>
                                                    </div>
                                                @else
                                                    @php
                                                    ...
                                                    @if(!empty($order) && $order->status == 1)
                                                        <div class="protip-btn">
                                                            <a href="{{ route('course.content',['slug' => $c->slug ]) }}" class="btn btn-secondary" title="{{ __('Go To Course') }}">{{ __('Go To Course') }}</a>
                                                        </div>
                                                    @else
                                                        @php
                                            ...
                                            @else
                                                <div class="protip-btn">                    
                                                    ...
                                                    <a href="{{ route('login') }}" class="btn btn-primary shopping-cart" type="{{ __('Add To Cart') }}"><i data-feather="shopping-cart"></i>&nbsp;{{ __('Add To Cart') }}</a>
                                        ...
                                        @else
                                             @if(Auth::check())
                                                ...
                                                @else
                                                    ...
                                                    @if($enroll == NULL)
                                                        <div class="protip-btn">
                                                            <a href="{{url('enroll/show',$c->id)}}" class="btn btn-primary" title="{{ __('Enroll Now') }}">{{ __('Enroll Now') }}</a>
                                                        </div>
                                                    @else
                                                        <div class="protip-btn">
                                                            <a href="{{ route('course.content',['slug' => $c->slug ]) }}" class="btn btn-secondary" title="{{ __('Go To Course') }}">{{ __('Go To Course') }}</a>
                                            ...
                                            @else
                                                <div class="protip-btn">
                                                    <a href="{{ route('login') }}" class="btn btn-primary" title="{{ __('Enroll Now') }}">{{ __('Enroll Now') }}</a>
                                                </div>
                                            @endif
                                        @endif
                                    </div>
                                                        ...
                                                        @if ($wish == NULL)
                                                            <li class="protip-wish-btn">
                                                                <form id="demo-form2" method="post" action="{{ url('show/wishlist', $c->id) }}" data-parsley-validate 
                                                                    ...
                                                                    <button class="wishlisht-btn"type="submit"><i data-feather="heart"></i></button>
                                                                </form>
                                                            </li>
                                                        @else
                                                            <li class="protip-wish-btn-two">
                                                                <form id="demo-form2" method="post" action="{{ url('remove/wishlist', $c->id) }}" data-parsley-validate 
                                                                    ...
                                                                    <button class="wishlisht-btn heart-fill" type="submit"><i data-feather="heart"></i></button>
                                                                </form>
                                                            </li>
                                                        @endif 
                                                    @else
